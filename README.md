...
Generate high-fidelity Discord chat transcripts in **HTML** or **JSON**  
with long-term archival support.
...
Discord does not provide a native way to export messages with visual fidelity for archival, moderation or legal purposes.

This library was built to generate structured transcripts that can be:
- stored long-term
- rendered later
- shared safely
- archived outside Discord

## 🧰 Common use cases

- Discord ticket systems  
- Moderation logs  
- Server backups  
- Evidence storage  
- Support transcripts  
...
This library generates HTML transcripts that may be opened in a browser.
To prevent XSS and malicious content injection, URLs and assets are sanitized by default.

**`safeMode` is enabled by default.**

When enabled, the library will:
- Sanitize image URLs
- Block unsafe protocols (data:, file:)
- Restrict external SVG images
- Resolve attachment:// safely
- Apply safeguards when downloading assets for base64 or CDN processing
- Escape all message text

Disabling safeMode - **NOT RECOMMENDED**
```
...
Disabling `safeMode` will bypass URL safety checks and allow any external image URL to be embedded or forwarded to configured CDNs.

⚠️ Only disable this if you fully trust the message source.
⚠️ Text content is always escaped and cannot be bypassed.
⚠️ Disabling can lead to XSS and SSRF risk
...
- `disableWarnings`: Disable all warnings to keep console output clean. **Warning**: Will hide issues like unsafe URLs or fallback usage. (Default: `false`)
...
- `safeMode`: Enables safe mode, blocking potentially unsafe URLs, images, or HTML from being included in the transcript. **Warning**: may allow unsafe content to appear. (Default: true)
...
Maintained by Henrique Mairesse  
