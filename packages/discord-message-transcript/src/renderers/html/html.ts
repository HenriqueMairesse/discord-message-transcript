import { Attachment, ButtonComponent, ButtonStyle, ChannelSelectMenuComponent, ComponentType, Embed, MentionableSelectMenuComponent, RoleSelectMenuComponent, StringSelectMenuComponent, TopLevelComponent, UserSelectMenuComponent } from "discord.js";
...
`
    ...
    private embedBuilder(message: JsonMessage, embeds: Embed[]): string {
        ...
        return components.map(component => {
            switch (component.type) {
                case ComponentType.ActionRow: {
                    ...
                    return `
                    <div class="actionRow">
                        ${component.components[0].type == ComponentType.Button ? component.components.map(button => {
                            return button.type == ComponentType.Button ? this.buttonBuilder(button) : "";
    ...
    private buttonBuilder(button: ButtonComponent): string {
        ...
        <div class="button" style="background-color: ${BUTTON_COLOR[button.style]}">
            ...
            <a class="buttonLink" href="${button.url}" target="_blank">
                ...
                <svg class="buttonLinkIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="#e3e3e3"><path d="M200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h280v80H200v560h560v-280h80v280q0 33-23.5 56.5T760-120H200Zm188-212-56-56 372-372H560v-80h280v280h-80v-144L388-332Z"/></svg>  
            </a>` : `
                ${button.emoji ? `<p class="buttonEmoji">${button.emoji}</p>` : ""}
                ${button.label ? `<p class="buttonLabel">${button.label}</p>` : ""}
            `}
    ...
    private selectorBuilder(selector: StringSelectMenuComponent | UserSelectMenuComponent | RoleSelectMenuComponent | MentionableSelectMenuComponent | ChannelSelectMenuComponent): string {
        return `
        <div class="selector">
            ...
            <div class="selectorOptionMenu">
                ${selector.type == ComponentType.StringSelect ? selector.options.map(option => {
                    return `
