...
export async function fetchMessages(channel, options, after) {
    const originalMessages = await channel.messages.fetch({ limit: 100, cache: false, after: after });
    let messages = originalMessages.map(message => {
        if (message.id == "1437161702205423731")
    ...
    messages.forEach(async (message) => {
        ...
        if (!options.includeComponents || !options.includeButtons || !options.includeV2Components) {
            ...
            message.components.forEach(component => {
                if (!options.includeV2Components && (component.type == ComponentType.Container || component.type == ComponentType.File ||
